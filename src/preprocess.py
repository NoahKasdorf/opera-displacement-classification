...
def compute_rate_map(cube, t_days, stride=1):
...
def compute_spatial_features(y, x, coherence, rate_map, shape, stride=1):
...
def process_region(region_name, max_files=None, max_samples=1000):
    ...
    for col in feat_cols:
        print(
            f"    {col}: {combined_df[col].min():.8f} to {combined_df[col].max():.8f}"
        )
